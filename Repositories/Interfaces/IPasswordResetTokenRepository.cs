...
namespace SubExplore.Repositories.Interfaces
{
    ...
    public interface IPasswordResetTokenRepository : IGenericRepository<PasswordResetToken>
    {
        ...
        /// <returns>Valid token if found</returns>
        Task<PasswordResetToken?> GetValidTokenByHashAndEmailAsync(string tokenHash, string email);
        ...
        Task<List<PasswordResetToken>> GetValidTokensForUserAsync(int userId);
        ...
        /// <summary>
        /// Check if user has reached maximum daily reset requests
        /// </summary>
        /// <param name="userId">User ID</param>
        /// <param name="maxResetsPerDay">Maximum resets per day</param>
        /// <returns>True if limit reached</returns>
        Task<bool> HasReachedDailyLimitAsync(int userId, int maxResetsPerDay = 5);

        /// <summary>
        /// Check if email has reached maximum daily reset requests (across all users)
        /// </summary>
        /// <param name="email">Email address</param>
        /// <param name="maxResetsPerDay">Maximum resets per day</param>
        /// <returns>True if limit reached</returns>
        Task<bool> HasEmailReachedDailyLimitAsync(string email, int maxResetsPerDay = 10);

        /// <summary>
        /// Get token statistics for monitoring
        /// </summary>
        /// <returns>Token usage statistics</returns>
        Task<PasswordResetTokenStatistics> GetTokenStatisticsAsync();
    }

    /// <summary>
    /// Password reset token statistics
    /// </summary>
    public class PasswordResetTokenStatistics
    {
