...
namespace SubExplore.Repositories.Implementations
{
    ...
    public class RevokedTokenRepository : GenericRepository<RevokedToken>, IRevokedTokenRepository
    {
        ...
        public async Task RevokeTokenAsync(string tokenHash, string tokenType, Guid? userId = null, 
        ...
        public async Task RevokeAllUserTokensAsync(Guid userId, string reason = RevocationReasons.UserLogout)
        {
            try
            {
        ...
        public async Task<List<RevokedToken>> GetUserRevokedTokensAsync(Guid userId, int limit = 50)
        {
            try
            {
