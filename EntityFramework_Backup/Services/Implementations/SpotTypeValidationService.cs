...
namespace SubExplore.Services.Implementations
{
    ...
    public class SpotTypeValidationService : ISpotTypeValidationService
    {
        ...
        public async Task<SpotTypeRepairResult> RepairSpotTypesAsync()
        {
            ...
            try
            {
                ...
                var defaultDivingType = await _context.SpotTypes
                    .FirstOrDefaultAsync(t => t.Category == ActivityCategory.Activity && t.IsActive);
        ...
        /// </summary>
        public async Task<string> GetDiagnosticReportAsync()
        {
            try
            {
                ...
                foreach (var type in activeSpotTypes)
                {
                    ...
                    report += $"  • {type.Name} ({categoryName}) - {type.ColorCode}\n";
                }
                report += $"Total actifs: {activeSpotTypes.Count}\n\n";

                // 2. Inactive SpotTypes
                var inactiveSpotTypes = await _context.SpotTypes
                    .Where(st => !st.IsActive)
                    .ToListAsync();
                    
                report += $"🗑️ TYPES INACTIFS: {inactiveSpotTypes.Count}\n";
                foreach (var type in inactiveSpotTypes)
                {
                    report += $"  • {type.Name}\n";
                }
                report += "\n";
                ...
                // 3. Spot distribution
                var spotCounts = await _context.Spots
                    .Where(s => s.Type != null && s.Type.IsActive)
                ...
                report += "📊 RÉPARTITION DES SPOTS PAR TYPE:\n";
                foreach (var count in spotCounts)
                {
                    report += $"  • {count.TypeName}: {count.Count} spots\n";
                }
                ...
                if (orphanedSpots > 0)
                {
                    report += $"\n⚠️ SPOTS ORPHELINS: {orphanedSpots}\n";
                    report += "  Ces spots ont des types inactifs ou manquants\n";
                    report += "  🔧 Recommandation: Utiliser RepairSpotTypesAsync()\n";
                }

                // 5. Category analysis
                var categoryCounts = await GetActiveTypeCountsByCategoryAsync();
                ...
                foreach (var category in categoryCounts)
                {
                    var categoryName = category.Key switch
                    {
                        ...
                        ActivityCategory.Shop => "Boutiques",
                        ActivityCategory.Other => "Autres",
                        _ => $"Cat{(int)category.Key}"
                    };
                    
                    report += $"  • {categoryName}: {category.Value} types actifs\n";
                    
                    if (category.Value == 0)
                    {
                        report += "    ⚠️ Aucun type actif dans cette catégorie!\n";
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                return $"❌ Erreur lors de la génération du diagnostic: {ex.Message}";
            }
        }
