...
namespace SubExplore.Services.Implementations
{
    public class UserProfileService : IUserProfileService
    {
        ...
        public Guid? CurrentUserId => _settingsService.Get<Guid?>("CurrentUserId");
        ...
        public async Task<UserStatsDto> GetUserStatsAsync(Guid userId)
        {
            try
            {
                ...
                if (user.Certifications != null && user.Certifications.Count > 0)
                {
                    try
                    {
                        ...
                        foreach (var cert in user.Certifications)
                        {
                            if (cert.Value != null)
                            {
                                certificationsList.Add(cert.Value.ToString());
                            }
                        }
                        stats.RecentCertifications = certificationsList.Take(3).ToList();
                    }
                    catch
                    {
