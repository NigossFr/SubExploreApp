...
namespace SubExplore.ViewModels.Spots
{
    public partial class SpotTypeItem : ObservableObject
    {
        ...
        public Dictionary<string, object> ValidationCriteria => SpotType.ValidationCriteria;
    ...
    public partial class AddSpotViewModel : ViewModelBase
    {
            ...
            : base(dialogService, navigationService)
        {
            ...
            {
                CreatorId = _authenticationService.CurrentUserId ?? Guid.NewGuid(), // Use authenticated user or fallback
                ...
                TypeId = Guid.Empty // Default value, will be updated when user selects
            };
        ...
        public override async Task InitializeAsync(object parameter = null)
        {
            ...
            // Load spot types asynchronously in background for better UI responsiveness
            _ = Task.Run(async () => await LoadSpotTypes().ConfigureAwait(false));
        ...
        /// <returns>SpotId if found, Guid.Empty otherwise</returns>
        private Guid TryExtractSpotIdFromParameter(object parameter)
        {
            if (parameter == null) return Guid.Empty;
            ...
            try
            {
                ...
                if (spotIdProperty != null)
                {
                    ...
                    if (spotIdValue != null && Guid.TryParse(spotIdValue.ToString(), out Guid spotId) && spotId != Guid.Empty)
                    {
                    ...
                    if (locationParamValue != null && !string.IsNullOrEmpty(locationParamValue.ToString()))
                    {
                        ...
                        foreach (var part in queryParts)
                        {
                            ...
                            if (keyValue.Length == 2)
                            {
                                ...
                                {
                                    if (Guid.TryParse(System.Web.HttpUtility.UrlDecode(keyValue[1]), out Guid spotId) && spotId != Guid.Empty)
                                    {
        ...
        /// <summary>
        /// Loads active spot types from repository for user selection with optimized threading
        /// </summary>
        ...
        /// <returns>Task representing the asynchronous loading operation</returns>
        private async Task LoadSpotForEditing(Guid spotId)
        {
            try
            {
        ...
        private void SaveCharacteristicsData()
        {
            ...
            NewSpot.TypeId = SelectedSpotTypes.Count > 0 ? SelectedSpotTypes[0].Id : Guid.Empty;
        ...
        private async Task SubmitSpot()
        {
            ...
            try
            {
                ...
                if (IsEditMode && EditingSpotId != Guid.Empty)
                {
                ...
                else
                {
                    ...
                    NewSpot.CreatorId = _authenticationService.CurrentUserId ?? Guid.NewGuid();
                ...
                // Retourner à la carte with optimized navigation
