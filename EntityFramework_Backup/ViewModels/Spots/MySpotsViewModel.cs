    ...
    public partial class MySpotsViewModel : ViewModelBase
    {
        ...
        public async Task DiagnoseAndRepairSpotTypesAsync()
        {
            try
            {
                ...
                var spotTypeValidation = _serviceProvider.GetService<SubExplore.Services.Interfaces.ISpotTypeValidationService>();
                var migrationService = _serviceProvider.GetService<SubExplore.Services.Implementations.SpotTypeMigrationService>();

                if (spotTypeValidation == null || migrationService == null)
                {
                ...
                var initialReport = await spotTypeValidation.GetDiagnosticReportAsync();
                ...
                var repairResult = await spotTypeValidation.RepairSpotTypesAsync();
                var repairSuccess = repairResult.Success;
                ...
                var finalReport = await spotTypeValidation.GetDiagnosticReportAsync();
                _logger.LogInformation("Final Diagnostic Report:\n{Report}", finalReport);

                var validationResult = await spotTypeValidation.ValidateSpotTypesAsync();
                var ecosystemValid = validationResult.IsValid;
