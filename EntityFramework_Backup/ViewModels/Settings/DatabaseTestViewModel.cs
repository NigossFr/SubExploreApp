...
namespace SubExplore.ViewModels.Settings
{
    public partial class DatabaseTestViewModel : ViewModelBase
    {
        ...
        // private readonly UpdateActivityCategoryStructure _categoryMigrationService; // Service supprimé pour PostgreSQL
        private readonly ISpotTypeValidationService _spotTypeValidationService;
        ...
        public DatabaseTestViewModel(IDatabaseService databaseService, DatabaseDiagnosticService diagnosticService, SpotTypeMigrationService migrationService, ISpotTypeValidationService spotTypeValidationService, ILogger<DatabaseTestViewModel> logger)
        {
            _databaseService = databaseService;
            _diagnosticService = diagnosticService;
            _migrationService = migrationService;
            // _categoryMigrationService = categoryMigrationService; // Service supprimé pour PostgreSQL
            _spotTypeValidationService = spotTypeValidationService;
            _logger = logger;
        ...
        private async Task ShowDatabaseDiagnosticsAsync()
        {
            try
            {
                ...
                // Use the unified diagnostic service for consistency
        ...
        private async Task RunUltraDeepDiagnosticAsync()
        {
            try
            {
                ...
                // Use the unified diagnostic service
                var diagnostics = await _diagnosticService.RunUltraDeepDiagnosticAsync();
                LogMessages += diagnostics + "\n";
                
                LogMessages += "✅ Diagnostic ultra-profond terminé avec succès\n";
            }
            catch (Exception ex)
            {
        ...
        private async Task FixActivityCategoryStructureAsync()
        {
            try
            {
                ...
                // await _categoryMigrationService.ExecuteAsync(); // Service supprimé pour PostgreSQL
                ...
                // var report = await _categoryMigrationService.GetStatusReportAsync(); // Service supprimé pour PostgreSQL
                var report = "Service de migration PostgreSQL non disponible";
                LogMessages += report + "\n";
        ...
        private async Task DiagnoseSpotTypesAsync()
        {
            try
            {
                ...
                var diagnostic = await _spotTypeValidationService.GetDiagnosticReportAsync();
        ...
        private async Task RepairSpotTypesAsync()
        {
            try
            {
                ...
                var repairResult = await _spotTypeValidationService.RepairSpotTypesAsync();
                var success = repairResult.Success;
